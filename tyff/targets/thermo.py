...
class SimulationConfig(pydantic.BaseModel):
    ...
    max_mols: int = pydantic.Field(..., description="The maximum number of molecules to simulate.")
    gen_coords: tyff.mm.GenerateCoordsConfig = pydantic.Field(
        ..., description="Configuration for generating initial coordinates."
    )

    apply_hmr: bool = pydantic.Field(False, description="Whether to apply hydrogen mass repartitioning.")

    equilibrate: list[tyff.mm.MinimizationConfig | tyff.mm.SimulationConfig] = pydantic.Field(
        ..., description="Configuration for equilibration simulations."
    )

    production: tyff.mm.SimulationConfig = pydantic.Field(
...
_SystemDict = dict[SimulationKey, tyff.TensorSystem]
...
def create_from_evaluator(dataset_file: pathlib.Path) -> datasets.Dataset:
    ...
    for phys_prop in property_data["properties"]:
        ...
        temp_unit = getattr(unit, phys_prop["thermodynamic_state"]["temperature"]["unit"])
        temp = phys_prop["thermodynamic_state"]["temperature"]["value"] * temp_unit
        pressure_unit = getattr(unit, phys_prop["thermodynamic_state"]["pressure"]["unit"])
...
def _convert_entry_to_system(
    entry: DataEntry, topologies: dict[str, tyff.TensorTopology], max_mols: int
) -> tuple[SimulationKey, tyff.TensorSystem]:
    ...
    system_topologies: list[tyff.TensorTopology] = [topologies[smiles_a]]
    n_copies = [n_copies_a]
    ...
    key = SimulationKey(tuple(smiles), tuple(n_copies), entry["temperature"], entry["pressure"])
    system = tyff.TensorSystem(system_topologies, n_copies, True)
...
def _bulk_config(temperature: float, pressure: float) -> SimulationConfig:
    ...
    return SimulationConfig(
        max_mols=256,
        gen_coords=tyff.mm.GenerateCoordsConfig(),
        equilibrate=[
            tyff.mm.MinimizationConfig(),
...
def default_config(phase: Phase, temperature: float, pressure: float | None) -> SimulationConfig:
...
def _plan_simulations(
    ...
    for entry in entries:
        ...
        bulk_config = select_config("bulk", entry["temperature"], entry["pressure"], simulation_config)
...
def _simulate(
    ...
    """
    coords, box_vectors = tyff.mm.generate_system_coords(system, force_field, config.gen_coords)
    ...
    with output_path.open("wb") as output:
        reporter = tyff.mm.TensorReporter(output, config.production_frequency, beta, config.production.pressure)
        tyff.mm.simulate(
            system,
            force_field,
            coords,
...
def _compute_observables(
    ...
    if cached_path is not None and cached_path.exists():
        with contextlib.suppress(tyff.mm.NotEnoughSamplesError):
            means = tyff.mm.reweight_ensemble_averages(system, force_field, cached_path, temperature, pressure)
            stds = {key: tyff.utils.tensor_like(torch.nan, means[key]) for key in means}
    ...
    return _Observables(*tyff.mm.compute_ensemble_averages(system, force_field, output_path, temperature, pressure))


def _predict_density(entry: DataEntry, observables: _Observables) -> tuple[torch.Tensor, torch.Tensor | None]:
...
def _predict_hvap(
    ...
    rt = (temperature * openmm.unit.MOLAR_GAS_CONSTANT_R).value_in_unit(openmm.unit.kilocalorie_per_mole)
...
def _predict_hmix(
    ...
    system_mix: tyff.TensorSystem,
    system_0: tyff.TensorSystem,
    system_1: tyff.TensorSystem,
    ...
    std = torch.sqrt(enthalpy_mix_std**2 + x_0**2 * enthalpy_0_std**2 + x_1**2 * enthalpy_1_std**2)
...
def predict(
    ...
    entries: list[DataEntry] = [*tyff.utils.dataset.iter_dataset(dataset)]  # type: ignore[list-item]

    required_simulations, entry_to_simulation = _plan_simulations(entries, topologies, simulation_config)
    observables = {
        phase: {
    ...
    for entry, keys in zip(entries, entry_to_simulation, strict=True):
        ...
        reference_std.append(torch.nan if entry["std"] is None else entry["std"] * abs(type_scale))
    ...
    reference = tyff.utils.tensor_like(reference, predicted)
    reference_std = tyff.utils.tensor_like(reference_std, predicted_std)
...
def default_closure(
    ...
    def closure_fn(
        ...
        y_ref, _, y_pred, _ = tyff.targets.thermo.predict(  # type: ignore[attr-defined]
            dataset,
            force_field,
            topologies,
            pathlib.Path.cwd(),
        ...
        if compute_hessian:
            hessian = tyff.utils.loss.approximate_hessian(x, y_pred)
        if compute_gradient:
