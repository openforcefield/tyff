...
EnergyFn = typing.Callable[["pandas.DataFrame", tuple[str, ...], torch.Tensor], torch.Tensor]
...
def create_from_des(
    ...
    for system_id in tqdm.tqdm(system_ids, desc="loading dimers"):
        ...
        for group_id in group_ids:
            ...
            smiles_a = tyff.utils.molecule.mol_to_smiles(mol_a, False)
            smiles_b = tyff.utils.molecule.mol_to_smiles(mol_b, False)

            source = f"{data_dir.name} system={system_id} orig={group_orig} group={group_id}"
...
def compute_dimer_energy(
    topology_a: tyff.TensorTopology,
    topology_b: tyff.TensorTopology,
    ...
    """
    dimer = tyff.TensorSystem([topology_a, topology_b], [1, 1], False)

    coords_a = coords[:, : topology_a.n_atoms, :]

    if topology_a.v_sites is not None:
        coords_a = tyff.geometry.add_v_site_coords(topology_a.v_sites, coords_a, force_field)

    coords_b = coords[:, topology_a.n_atoms :, :]

    if topology_b.v_sites is not None:
        coords_b = tyff.geometry.add_v_site_coords(topology_b.v_sites, coords_b, force_field)

    coords = torch.cat([coords_a, coords_b], dim=1)

    energy_dimer = tyff.compute_energy(dimer, force_field, coords)

    energy_a = tyff.compute_energy(topology_a, force_field, coords_a)
    energy_b = tyff.compute_energy(topology_b, force_field, coords_b)
...
def _predict(
    ...
    coords_flat = tyff.utils.tensor_like(dimer["coords"], force_field.potentials[0].parameters)
    ...
    reference = tyff.utils.tensor_like(dimer["energy"], predicted)
...
def predict(
    ...
    reference, predicted = zip(
        *[_predict(dimer, force_field, topologies) for dimer in tyff.utils.dataset.iter_dataset(dataset)],
        strict=True,
    )
...
def default_closure(
    ...
    def loss_fn(_x: torch.Tensor) -> torch.Tensor:
        y_ref, y_pred = tyff.targets.dimers.predict(  # type: ignore[attr-defined]
            dataset, trainable.to_force_field(_x), topologies
        )
        return ((y_pred - y_ref) ** 2).sum()

    return tyff.utils.loss.to_closure(loss_fn)
...
def _plot_energies(energies: dict[str, torch.Tensor]) -> str:
    ...
    for i, (k, v) in enumerate(energies.items()):
        axis.plot(
            ...
            label=k,
            linestyle="none",
            marker=tyff.utils.reporting.DEFAULT_MARKERS[i],
            color=tyff.utils.reporting.DEFAULT_COLORS[i],
        )
    ...
    figure.tight_layout()
    img = tyff.utils.reporting.figure_to_img(figure)
...
def report(
    dataset: datasets.Dataset,
    force_fields: dict[str, tyff.TensorForceField],
    topologies: dict[str, dict[str, tyff.TensorTopology]],
    ...
    delta_sqr_total = {force_field_name: torch.zeros(1) for force_field_name in force_fields}
    delta_sqr_count = 0

    for dimer in tyff.utils.dataset.iter_dataset(dataset):
        ...
        mol_img = tyff.utils.reporting.mols_to_img(dimer["smiles_a"], dimer["smiles_b"])
    ...
    rmse_total_rows = [
        {
            ...
            "RMSE [kcal/mol]": torch.sqrt(delta_sqr_total[force_field_name].sum() / delta_sqr_count).item(),
        }
        for force_field_name in force_fields
    ]
    ...
    formatters_stats = {col: rmse_format for col in data_stats.columns if col.startswith("RMSE")}
    formatters_full = {
