...
def broadcast_parameters(system: tyff.TensorSystem, potential: tyff.TensorPotential) -> torch.Tensor:
...
def broadcast_idxs(system: tyff.TensorSystem, potential: tyff.TensorPotential) -> torch.Tensor:
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        offset = idx_offset + tyff.utils.arange_like(n_copies, idxs) * topology.n_particles
...
) -> tuple[tyff.TensorSystem, torch.Tensor, torch.Tensor | None]:
...
def _precompute_pairwise(
    ...
    conformer: torch.Tensor,
    box_vectors: torch.Tensor | None = None,
) -> typing.Optional["tyff.potentials.nonbonded.PairwiseDistances"]:
    ...
    for potential in force_field.potentials:
        ...
        if tyff.CUTOFF_ATTRIBUTE in potential.attribute_cols:
    ...
    return tyff.potentials.nonbonded.compute_pairwise(system, conformer, box_vectors, cutoff)
...
def compute_energy_potential(
    ...
    conformer: torch.Tensor,
    box_vectors: torch.Tensor | None = None,
    pairwise: typing.Optional["tyff.potentials.nonbonded.PairwiseDistances"] = None,
) -> torch.Tensor:
    ...
    importlib.import_module("tyff.potentials.nonbonded")
    importlib.import_module("tyff.potentials.valence")
...
def compute_energy(
    ...
    energy = tyff.utils.zeros_like(conformer.shape[0] if conformer.ndim == 3 else 1, conformer)
