...
def _broadcast_exclusions(
    system: tyff.TensorSystem, potential: tyff.TensorPotential
...
def compute_pairwise_scales(system: tyff.TensorSystem, potential: tyff.TensorPotential) -> torch.Tensor:
    ...
    pair_scales = tyff.utils.ones_like(n_pairs, other=potential.parameters)

    if len(exclusion_idxs) > 0:
        exclusion_idxs, _ = exclusion_idxs.sort(dim=1)  # ensure upper triangle

        pair_idxs = tyff.utils.to_upper_tri_idx(exclusion_idxs[:, 0], exclusion_idxs[:, 1], n_particles)
...
def prepare_lrc_types(
    ...
    counts = tyff.utils.tensor_like([n_by_type[key] for key in potential.parameter_keys], potential.parameters)
...
def lorentz_berthelot(
    ...
    Returns:
        ...
        ``shape=(n_pairs, 1)``.
    """
    return tyff.utils.geometric_mean(epsilon_a, epsilon_b), 0.5 * (sigma_a + sigma_b)
...
def _compute_switch_fn(
    ...
    if tyff.SWITCH_ATTRIBUTE not in potential.attribute_cols:
        return torch.ones(1), None

    switch_width_idx = potential.attribute_cols.index(tyff.SWITCH_ATTRIBUTE)
...
def _integrate_lj_switch(
    ...
    coeff_0 = tyff.utils.tensor_like([rs**3, rs**2, rs, 1, b, b**2], rs) * (
        tyff.utils.tensor_like([rs**2 * b**2, rs * b, 1], rs)
        * tyff.utils.tensor_like([[6, 15, 10], [1, 2, 1], [2, 3, 1], [6, 6, 1], [0, 2, 1], [0, 0, 1]], rs)
    ).sum(dim=-1)

    coeff_01 = sig[:, None] ** 6 * tyff.utils.tensor_like([-28, 945, -1080, 420, -756, 378], rs) * coeff_0
    coeff_11 = tyff.utils.tensor_like([84, -3780, 7560, 2520, -3780, 756], rs) * coeff_0

    powers = tyff.utils.tensor_like([-9, -8, -7, -6, -5, -4, -3, -2, -1, 0, 1, 2], rs)
...
def _compute_lj_lrc(
    ...
    idxs_i, idxs_j, n_ij_interactions = tyff.potentials.nonbonded.prepare_lrc_types(system, potential)
    ...
    eps_ij, sig_ij = tyff.potentials.nonbonded.lorentz_berthelot(
...
@tyff.potentials.potential_energy_fn(tyff.PotentialType.VDW, tyff.EnergyFn.VDW_LJ)
def compute_lj_energy(
    ...
    cutoff = potential.attributes[potential.attribute_cols.index(tyff.CUTOFF_ATTRIBUTE)]
    ...
    pairs_1d = tyff.utils.to_upper_tri_idx(pairwise.idxs[:, 0], pairwise.idxs[:, 1], len(parameters))
    ...
    if potential.exceptions is not None:
        exception_idxs, exceptions = tyff.potentials.broadcast_exceptions(
...
def _integrate_dexp_switch(
    ...
    """
    rs_pow = tyff.utils.tensor_like([rs**5, rs**4, rs**3, rs**2, rs, 1, 0, 0], rc).unsqueeze(1)

    # fmt: off
    c_n = tyff.utils.tensor_like(
        [
            [  6,  15,  10],
            [-30, -60, -30],
            [ 60,  90,  30],
    ...
    b = b.unsqueeze(1)
    b_pow = torch.hstack(
        [tyff.utils.zeros_like((len(b), 1), b)] * 4
    ...
    # fmt: off
    mat_coeff = tyff.utils.tensor_like(
        [
            [-2,    -2,    -1,     0,    0,    0,   0,  0],
            [-6,    -6,    -3,    -1,    0,    0,   0,  0],
            [-24,   -24,   -12,   -4,   -1,    0,   0,  0],
...
def _compute_dexp_lrc(
    ...
    eps_ij, r_min_ij = tyff.potentials.nonbonded.lorentz_berthelot(
...
@tyff.potentials.potential_energy_fn(tyff.PotentialType.VDW, tyff.EnergyFn.VDW_DEXP)
def compute_dexp_energy(
    ...
    eps, r_min = tyff.potentials.nonbonded.lorentz_berthelot(
        parameters[pairwise.idxs[:, 0], eps_column],
        parameters[pairwise.idxs[:, 1], eps_column],
...
def _compute_pme_exclusions(system: tyff.TensorSystem, potential: tyff.TensorPotential) -> torch.Tensor:
...
def _compute_coulomb_energy_periodic(
    ...
    charges = tyff.potentials.broadcast_parameters(system, potential).squeeze(-1)
...
@tyff.potentials.potential_energy_fn(tyff.PotentialType.ELECTROSTATICS, tyff.EnergyFn.COULOMB)
