...
import tyff
import tyff.potentials.nonbonded
import tyff.utils
...
def get_test_data_path(path: str | pathlib.Path) -> pathlib.Path:
    ...
    path : str | pathlib.Path
        ...
        relative to the `tyff/_tests/data` directory.
...
def mock_convert_fn_with_deps(
    ...
    v_site_maps: list[tyff.VSiteMap | None],
    dependencies: dict[str, tuple[tyff.TensorPotential, list[tyff.NonbondedParameterMap]]],
) -> tuple[tyff.TensorPotential, list[tyff.NonbondedParameterMap]]:
    raise NotImplementedError()


def convert_lj_to_dexp(potential: tyff.TensorPotential):
...
def topology_from_smiles(smiles: str) -> tyff.TensorTopology:
...
def system_from_smiles(
...
) -> tuple[tyff.TensorSystem, tyff.TensorForceField]:
    ...
    return tyff.TensorSystem(tensor_tops, n_copies, is_periodic=True), tensor_ff


def _parameter_key_to_idx(potential: tyff.TensorPotential, key: str):
    return next(iter(i for i, k in enumerate(potential.parameter_keys) if k.id == key))


def system_with_exceptions() -> tuple[tyff.TensorSystem, tyff.TensorPotential, dict[str, LJParam]]:
    ...
    system, tensor_ff = tyff._tests.utils.system_from_smiles(["O", "[Ar]"], [1, 1], ff)
    ...
    for top in system.topologies:
        assignment_matrix = tyff.utils.zeros_like(
...
def add_explicit_lb_exceptions(potential: tyff.TensorPotential, system: tyff.TensorSystem):
    ...
    eps_ij, sig_ij = tyff.potentials.nonbonded.lorentz_berthelot(eps_i, eps_j, sig_i, sig_j)
