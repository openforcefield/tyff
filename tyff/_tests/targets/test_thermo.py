...
from tyff.targets.thermo import (
    DataEntry,
    SimulationConfig,
    SimulationKey,
...
@pytest.mark.parametrize("phase, pressure, expected_n_mols", [("bulk", 1.23, 256), ("vacuum", None, 1)])
def test_default_config(phase, pressure, expected_n_mols):
    ...
    assert config.production.temperature.value_in_unit(openmm.unit.kelvin) == expected_temperature
...
@pytest.mark.parametrize("max_mols", [pytest.param(256, id="256"), pytest.param(1000, id="1000")])
def test_plan_simulations(mock_density_pure, mock_density_binary, mock_hvap, mock_hmix, mocker, max_mols):
    ...
    custom_config = {
        "bulk": SimulationConfig(
            max_mols=max_mols,
            gen_coords=tyff.mm.GenerateCoordsConfig(),
            equilibrate=[tyff.mm.MinimizationConfig()],
            production=tyff.mm.SimulationConfig(
                temperature=300 * openmm.unit.kelvin,
                pressure=None,
                n_steps=5000,
    ...
    assert required_simulations["vacuum"][expected_vacuum_key].topologies == [topology_cccc]
...
def test_simulation(tmp_cwd, mocker):
    ...
    mock_gen_coords = mocker.patch(
        "tyff.mm.generate_system_coords",
        autospec=True,
        return_value=(coords, box_vectors),
    )
    mock_simulate = mocker.patch("tyff.mm.simulate", autospec=True)

    spied_reporter = mocker.spy(tyff.mm, "TensorReporter")
    ...
    expected_beta = 1.0 / (openmm.unit.MOLAR_GAS_CONSTANT_R * expected_temperature * openmm.unit.kelvin)
...
def test_compute_observables_reweighted(tmp_cwd, mocker):
    ...
    mock_reweight = mocker.patch("tyff.mm.reweight_ensemble_averages", autospec=True, return_value=mock_result)
    ...
    mock_reweight.assert_called_once_with(mock_system, mock_ff, expected_path, 298.15 * openmm.unit.kelvin, None)


def test_compute_observables_simulated(tmp_cwd, mocker):
    mock_result = {"density": torch.randn(1)}, {"density": torch.randn(1)}
    mocker.patch(
        "tyff.mm.reweight_ensemble_averages",
        autospec=True,
        side_effect=tyff.mm._ops.NotEnoughSamplesError(),
    )
    mock_simulate = mocker.patch("tyff.targets.thermo._simulate", autospec=True)
    mock_compute = mocker.patch("tyff.mm.compute_ensemble_averages", autospec=True, return_value=mock_result)
    ...
    mock_simulate.assert_called_once_with(mock_system, mock_ff, mocker.ANY, expected_path)
    mock_compute.assert_called_once_with(mock_system, mock_ff, expected_path, 298.15 * openmm.unit.kelvin, None)
...
def test_predict_density(mock_density_pure, mocker):
    ...
    observables = {"bulk": {key: _Observables({"density": expected_result}, {"density": expected_std})}}
...
def test_predict_hvap(mock_hvap, mocker):
    ...
    system_vacuum = tyff.TensorSystem([topologies["CCCC"]], [1], False)
    ...
    rt = (mock_hvap["temperature"] * openmm.unit.kelvin * openmm.unit.MOLAR_GAS_CONSTANT_R).value_in_unit(
...
def test_predict_hmix(mock_hmix, mocker):
    ...
    key_0 = SimulationKey(("CCO",), (n_mols,), mock_hmix["temperature"], mock_hmix["pressure"])
    key_1 = SimulationKey(("CO",), (n_mols,), mock_hmix["temperature"], mock_hmix["pressure"])

    system_0 = tyff.TensorSystem([topologies["CCO"]], [n_mols], False)
    system_1 = tyff.TensorSystem([topologies["CO"]], [n_mols], False)
    ...
    averages = {
        "bulk": {
            key_bulk: _Observables({"enthalpy": enthalpy_bulk}, {"enthalpy": enthalpy_bulk_std}),
...
def test_predict(tmp_cwd, mock_density_pure, mocker):
    ...
    mock_compute = mocker.patch(
        "tyff.targets.thermo._compute_observables",
        autospec=True,
        return_value=_Observables({"density": mock_density}, {"density": mock_density_std}),
    )
...
def test_unsupported_property(data_dir):
    with pytest.raises(KeyError):
        _ = create_from_evaluator(dataset_file=data_dir / "missing_property_evaluator.json")
