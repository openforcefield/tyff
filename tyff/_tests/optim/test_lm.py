...
def test_damping_factor_loss_fn(mocker):
    ...
    solver_fn = mocker.patch("tyff.optim._lm._solver", autospec=True, return_value=(dx, 0.0))

    trust_radius = 12

    difference = _damping_factor_loss_fn(damping_factor, gradient, hessian, trust_radius)
...
def test_levenberg_marquardt_adaptive(mocker, caplog):
    ...
    mock_step_fn = mocker.patch("tyff.optim._lm._step", autospec=True, side_effect=mock_dx_traj)
    ...
    with caplog.at_level(logging.INFO):
        x_new = levenberg_marquardt(x, LevenbergMarquardtConfig(max_steps=3), mock_loss_fn, None)
...
def test_levenberg_marquardt(mode, n_steps, expected_n_grad_calls, expected_n_hess_calls):
