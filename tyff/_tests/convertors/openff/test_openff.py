...
import tyff
from tyff.converters.openff._openff import (
    _CONVERTERS,
    _convert_topology,
    _Converter,
...
def test_convert_handler(ethanol, ethanol_interchange, mocker):
    ...
    mock_convert = mocker.patch(
        "tyff._tests.utils.mock_convert_fn_with_deps",
        autospec=True,
    ...
    v_site_maps = [tyff.VSiteMap([v_site], {v_site: ethanol.n_atoms}, torch.tensor([[0]]))]
...
def test_convert_topology(formaldehyde, mocker):
    ...
    v_sites = tyff.VSiteMap([], {}, torch.tensor([]))
    constraints = tyff.TensorConstraints(torch.tensor([1, 2]), torch.tensor([3.0]))
