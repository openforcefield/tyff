...
def test_apply_hmr():
    ...
    force_field, [topology_water, topology_meoh] = tyff.converters.convert_interchange(interchanges)
    ...
    system_openmm = tyff.converters.convert_to_openmm_system(force_field, system_tyff)
    ...
    _apply_hmr(system_openmm, system_tyff)
...
def test_topology_to_xyz(mocker):
    ...
    mocker.patch("tyff.mm._utils.topology_to_rdkit", return_value=mock_molecule)

    interchange = openff.interchange.Interchange.from_smirnoff(
        openff.toolkit.ForceField("tip4p_fb.offxml"),
        openff.toolkit.Molecule.from_rdkit(mock_molecule).to_topology(),
    )
    force_field, [topology] = tyff.converters.convert_interchange(interchange)
...
def test_approximate_box_size():
    system = tyff.TensorSystem([tyff._tests.utils.topology_from_smiles("O")], [256], True)

    config = tyff.mm.GenerateCoordsConfig(scale_factor=2.0)
...
def test_generate_packmol_input():
    ...
    config = tyff.mm.GenerateCoordsConfig(tolerance=expected_tolerance, seed=expected_seed)
...
def test_generate_system_coords():
    coords, box_vectors = generate_system_coords(
        tyff.TensorSystem(
            [
                tyff._tests.utils.topology_from_smiles("O"),
                tyff._tests.utils.topology_from_smiles("CO"),
            ],
            [1, 2],
            True,
        ),
        None,
        tyff.mm.GenerateCoordsConfig(),
    )
...
def test_generate_system_coords_with_v_sites():
    ...
    force_field, [topology] = tyff.converters.convert_interchange(interchange)
    system = tyff.TensorSystem([topology], [1], False)
...
def test_simulate(mocker, mock_argon_tensors):
    ...
    spied_energy_minimize = mocker.spy(tyff.mm._mm, "_energy_minimize")
    spied_run_simulation = mocker.spy(tyff.mm._mm, "_run_simulation")
    ...
    simulate(
        ...
        [
            tyff.mm.MinimizationConfig(),
            tyff.mm.SimulationConfig(temperature=86.0 * openmm.unit.kelvin, pressure=None, n_steps=1),
        ],
        tyff.mm.SimulationConfig(temperature=86.0 * openmm.unit.kelvin, pressure=None, n_steps=2),
        [reporter],
        True,
    )
