...
from tyff.mm._reporters import _encoder


def _mock_potential(type_, parameters, attributes) -> tyff.TensorPotential:
    return tyff.TensorPotential(
        type_,
        f"{type_}-fn",
        parameters,
...
def test_pack_unpack_force_field(mocker):
    ...
    force_field = tyff.TensorForceField(
        ...
        tyff.TensorVSites(v_site_keys, v_site_weights, v_site_parameters),
    )
...
def test_compute_mass():
    water = tyff._tests.utils.topology_from_smiles("O")
    methane = tyff._tests.utils.topology_from_smiles("C")

    system = tyff.TensorSystem([water, methane], [2, 3], True)
...
def test_compute_frame_observables_non_periodic(mocker):
    system = tyff.TensorSystem([tyff._tests.utils.topology_from_smiles("[Ar]")], [1], False)
...
def test_compute_frame_observables():
    system = tyff.TensorSystem([tyff._tests.utils.topology_from_smiles("[Ar]")], [1], True)
...
def test_compute_observables(tmp_path, mock_argon_tensors, mock_argon_params):
    ...
    tensor_system = tyff.TensorSystem([tensor_top], [2], False)
...
def test_compute_ensemble_averages(mocker, tmp_path, mock_argon_tensors):
    ...
    mock_compute_observables = mocker.patch(
        "tyff.mm._ops._compute_observables",
        autospec=True,
