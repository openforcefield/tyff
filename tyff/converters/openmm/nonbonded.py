...
def _create_nonbonded_force[T: openmm.NonbondedForce | openmm.CustomNonbondedForce](
    ...
    cutoff_idx = potential.attribute_cols.index(tyff.CUTOFF_ATTRIBUTE)
    switch_idx = (
        None
        if tyff.SWITCH_ATTRIBUTE not in potential.attribute_cols
        else potential.attribute_cols.index(tyff.SWITCH_ATTRIBUTE)
    )
...
def _detect_parameters(
    potential: tyff.TensorPotential, energy_fn: str, mixing_fn: dict[str, str]
...
def _extract_parameters(potential: tyff.TensorPotential, parameter: torch.Tensor, cols: list[str]) -> list[float]:
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        for _ in range(n_copies):
            ...
            for index, (i, j) in enumerate(parameter_map.exclusions):
                ...
                if torch.isclose(scale, tyff.utils.tensor_like(0.0, scale)):
                    continue
...
@tyff.converters.openmm.potential_converter(tyff.PotentialType.VDW, tyff.EnergyFn.VDW_LJ)
def convert_lj_potential(
    potential: tyff.TensorPotential, system: tyff.TensorSystem
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        ...
        for _ in range(n_copies):
            ...
            for index, (i, j) in enumerate(parameter_map.exclusions):
                ...
                eps, sig = tyff.potentials.nonbonded.lorentz_berthelot(eps_i, eps_j, sig_i, sig_j)
...
@tyff.converters.openmm.potential_converter(tyff.PotentialType.VDW, tyff.EnergyFn.VDW_DEXP)
...
@tyff.converters.openmm.potential_converter(tyff.PotentialType.ELECTROSTATICS, tyff.EnergyFn.COULOMB)
def convert_coulomb_potential(potential: tyff.TensorPotential, system: tyff.TensorSystem) -> openmm.NonbondedForce:
