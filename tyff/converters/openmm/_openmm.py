...
_CONVERTER_FUNCTIONS: dict[
    tuple[str, str],
    typing.Callable[[tyff.TensorPotential, tyff.TensorSystem], openmm.Force | list[openmm.Force]],
] = {}
...
def create_openmm_system(system: tyff.TensorSystem, v_sites: tyff.TensorVSites | None) -> openmm.System:
    """Create an empty OpenMM system from a ``tyff`` system."""
    ...
    for topology, n_copies in zip(system.topologies, system.n_copies, strict=True):
        for _ in range(n_copies):
            ...
            for key, parameter_idx in zip(topology.v_sites.keys, topology.v_sites.parameter_idxs, strict=True):
                ...
                local_frame_coords = tyff.geometry.polar_to_cartesian_coords(
...
def _apply_constraints(omm_system: openmm.System, system: tyff.TensorSystem):
    idx_offset = 0
...
def convert_to_openmm_force(potential: tyff.TensorPotential, system: tyff.TensorSystem) -> list[openmm.Force]:
    """Convert a ``tyff`` potential to OpenMM forces.
    ...
    importlib.import_module("tyff.converters.openmm.nonbonded")
    importlib.import_module("tyff.converters.openmm.valence")
...
def convert_to_openmm_system(
    ...
    """Convert a ``tyff`` force field and system / topology into an OpenMM system.
...
def convert_to_openmm_topology(
    ...
    """Convert a ``tyff`` system to an OpenMM topology.
