"""Convert tyff parameters to OpenMM ForceField XML files."""
...
_CONVERTER_FUNCTIONS: dict[
    tuple[str, str],
    typing.Callable[[tyff.TensorPotential, tyff.TensorTopology, list[str]], ElementTree.Element],
] = {}
...
def ffxml_converter(potential_type: str, energy_expression: str):
    ...
    The decorated function should take a `tyff.TensorPotential`, and
    the associated `tyff.ParameterMap` and list of atom types, and return a
...
def _get_param_col(pot: tyff.TensorPotential, param_name: str) -> tuple[int, float]:
...
def _to_residue(
    top: tyff.TensorTopology,
    ff: tyff.TensorForceField,
    ...
    for v_site_idx, param_idx in enumerate(top.v_sites.parameter_idxs.tolist()):
        local_frame_coords = (
            tyff.geometry.polar_to_cartesian_coords(ff.v_sites.parameters[[param_idx], :].detach()) * _ANGSTROM_TO_NM
        )
...
def default_valence_converter(
    pot: tyff.TensorPotential,
    param_map: tyff.ValenceParameterMap,
    atom_types: list[str],
...
@ffxml_converter(tyff.PotentialType.VDW, tyff.EnergyFn.VDW_LJ)
def convert_lj_force(
    pot: tyff.TensorPotential,
    param_map: tyff.NonbondedParameterMap,
    types: list[str],
) -> ElementTree.Element:
...
@ffxml_converter(tyff.PotentialType.ELECTROSTATICS, tyff.EnergyFn.COULOMB)
...
@ffxml_converter(tyff.PotentialType.BONDS, tyff.EnergyFn.BOND_HARMONIC)
def convert_bond_potential(pot: tyff.TensorPotential, param_map: tyff.ValenceParameterMap, types: list[str]):
    return default_valence_converter(pot, param_map, types, ("k", "length"), "HarmonicBondForce", "Bond")


@ffxml_converter(tyff.PotentialType.ANGLES, tyff.EnergyFn.ANGLE_HARMONIC)
def convert_angle_potential(pot: tyff.TensorPotential, param_map: tyff.ValenceParameterMap, types: list[str]):
...
def _convert_to_openmm_ffxml(
    ...
    omm_top = tyff.converters.convert_to_openmm_topology(top)
    ...
    for atom in omm_top.atoms():
        ...
        atom_name_to_type[atom.name] = f"tyff-{short_uuid}-{atom_type}"
...
def convert_to_openmm_ffxml(
    ...
    system: tyff.TensorSystem | tyff.TensorTopology,
) -> list[str]:
    """Convert a tyff force field and system to OpenMM force field XML
    representations.
    ...
    """
    if isinstance(system, tyff.TensorTopology):
        system = tyff.TensorSystem([system], [1], False)
