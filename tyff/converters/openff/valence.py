...
def strip_constrained_bonds(
    parameter_maps: list[tyff.ValenceParameterMap],
...
def convert_valence_handlers(
...
) -> tuple[tyff.TensorPotential, list[tyff.ValenceParameterMap]]:
    ...
    """
    potential = tyff.converters.openff._openff._handlers_to_potential(handlers, handler_type, parameter_cols, None)
    ...
    for handler in handlers:
        ...
        parameter_map = tyff.ValenceParameterMap(torch.tensor(particle_idxs), assignment_matrix.to_sparse())
        parameter_maps.append(parameter_map)

    return potential, parameter_maps


@tyff.converters.smirnoff_parameter_converter("Bonds", {"k": _KCAL_PER_MOL / _ANGSTROM**2, "length": _ANGSTROM})
def convert_bonds(
...
@tyff.converters.smirnoff_parameter_converter("Angles", {"k": _KCAL_PER_MOL / _RADIANS**2, "angle": _RADIANS})
def convert_angles(
...
@tyff.converters.smirnoff_parameter_converter(
    "ProperTorsions",
    {
        "k": _KCAL_PER_MOL,
