...
def convert_nonbonded_handlers(
    ...
    potential = tyff.converters.openff._openff._handlers_to_potential(
        handlers,
        handler_type,
        parameter_cols,
        attribute_cols,
    ...
    for handler, topology, v_site_map in zip(handlers, topologies, v_site_maps, strict=True):
        ...
        if has_exclusions:
            exclusion_to_scale = tyff.utils.find_exclusions(topology, v_site_map)
        ...
        parameter_map = tyff.NonbondedParameterMap(
...
def convert_dexp(
    ...
    (
        potential,
        parameter_maps,
    ) = tyff.converters.openff.nonbonded.convert_nonbonded_handlers(
        handlers,
        "DoubleExponential",
        topologies,
        v_site_maps,
...
def _convert_charge_increment_handlers(
    ...
    potential_bci = tyff.converters.openff._openff._handlers_to_potential(
