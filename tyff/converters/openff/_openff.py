...
def _convert_v_sites(
...
) -> tuple[tyff.TensorVSites, list[tyff.VSiteMap | None]]:
    ...
    parameters = [
        [
            _get_value(
                parameters_by_key[parameter_key],
                column,
                tyff.TensorVSites.default_units(),
    ...
    v_site_frames = [
        tyff.geometry.V_SITE_TYPE_TO_FRAME[parameter_key_to_type[parameter_key]] for parameter_key in parameter_keys
    ]
    v_sites = tyff.TensorVSites(keys=parameter_keys, weights=v_site_frames, parameters=torch.tensor(parameters))
...
def _convert_constraints(
...
) -> list[tyff.TensorConstraints | None]:
    handler_types = {handler.type for handler in handlers}
    ...
    for handler in handlers:
        ...
        constraint = tyff.TensorConstraints(idxs=constraint_idxs, distances=torch.tensor(constraint_distances))
...
def convert_handlers(
    handlers: list[openff.interchange.smirnoff.SMIRNOFFCollection],
    topologies: list[openff.toolkit.Topology],
    v_site_maps: list[tyff.VSiteMap | None] | None = None,
    potentials: (list[tuple[tyff.TensorPotential, list[tyff.ParameterMap]]] | None) = None,
    constraints: list[tyff.TensorConstraints | None] | None = None,
) -> list[tuple[tyff.TensorPotential, list[tyff.ParameterMap]]]:
    ...
    """
    importlib.import_module("tyff.converters.openff.nonbonded")
    importlib.import_module("tyff.converters.openff.valence")
...
def _convert_topology(
    ...
    parameters: dict[str, tyff.ParameterMap],
    v_sites: tyff.VSiteMap | None,
    constraints: tyff.TensorConstraints | None,
) -> tyff.TensorTopology:
...
def convert_interchange(
    ...
    tensor_force_field = tyff.TensorForceField(potentials, v_sites)
