...
import tyff.mm._config
import tyff.mm._reporters
import tyff.mm._utils

_LOGGER = logging.getLogger("tyff.mm")
...
def _apply_hmr(
    ...
    Args:
        ...
        system_tyff: The tyff system that defines the system topology.
        hydrogen_mass: The mass to use for hydrogen atoms.
    """

    idx_offset = 0

    for topology, n_copies in zip(system_tyff.topologies, system_tyff.n_copies, strict=True):
...
def _topology_to_xyz(topology: tyff.TensorTopology, force_field: tyff.TensorForceField | None) -> str:
    """Convert a topology to an RDKit molecule."""
    mol = tyff.mm._utils.topology_to_rdkit(topology)
    ...
    if topology.n_v_sites > 0:
        ...
        coords = tyff.geometry.add_v_site_coords(topology.v_sites, coords, force_field)
...
def _approximate_box_size(
    ...
    config: "tyff.mm.GenerateCoordsConfig",
...
def generate_system_coords(
    system: tyff.TensorSystem,
    force_field: tyff.TensorForceField | None,
    config: typing.Optional["tyff.mm.GenerateCoordsConfig"] = None,
    ...
    config = config if config is not None else tyff.mm.GenerateCoordsConfig()
...
def _energy_minimize(
    ...
    config: "tyff.mm.MinimizationConfig",
) -> openmm.State:
...
def _run_simulation(
    ...
    state: openmm.State | tuple[openmm.unit.Quantity, openmm.unit.Quantity],
    platform: openmm.Platform,
    config: "tyff.mm.SimulationConfig",
...
def simulate(
    ...
    equilibrate_configs: list[typing.Union["tyff.mm.MinimizationConfig", "tyff.mm.SimulationConfig"]],
    production_config: "tyff.mm.SimulationConfig",
    production_reporters: list[typing.Any] | None = None,
    apply_hmr: bool = False,
) -> openmm.State:
    """Simulate a tyff system of molecules or topology.
    ...
    system: tyff.TensorSystem = (
        system if isinstance(system, tyff.TensorSystem) else tyff.TensorSystem([system], [1], False)
    ).to("cpu")

    requires_pbc = any(
        config.pressure is not None
        for config in [*equilibrate_configs, production_config]
        if isinstance(config, tyff.mm.SimulationConfig)
    )
    ...
    omm_system = tyff.converters.convert_to_openmm_system(force_field, system)
    omm_topology = tyff.converters.convert_to_openmm_topology(system)
    ...
    for i, config in enumerate(equilibrate_configs):
        ...
        if isinstance(config, tyff.mm.MinimizationConfig):
            omm_state = _energy_minimize(omm_system, omm_state, platform, config)

        elif isinstance(config, tyff.mm.SimulationConfig):
