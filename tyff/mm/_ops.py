...
def _pack_force_field(
    ...
    """Pack a tyff force field into a tuple that can be consumed by a custom PyTorch op.
...
def _unpack_force_field(
    ...
    """Unpack a tyff force field from its packed tensor and lookup representation.
    ...
    if has_v_sites:
        v_sites = tyff.TensorVSites(force_field.v_sites.keys, force_field.v_sites.weights, tensors[-1])

    return tyff.TensorForceField(potentials, v_sites)


def _compute_mass(system: tyff.TensorSystem) -> float:
...
def _compute_observables(
    ...
    for coords, box_vectors, _, kinetic in tyff.mm._reporters.unpack_frames(frames_file):
        coords = coords.to(theta[0].device)
        box_vectors = box_vectors.to(theta[0].device)

        with torch.enable_grad():
            potential = tyff.compute_energy(system, force_field, coords, box_vectors)
        ...
        if len(needs_grad) > 0:
            du_d_theta_subset = torch.autograd.grad(
                ...
                [tyff.utils.ones_like(1, potential)],
                retain_graph=False,
                allow_unused=True,
            )
    ...
    reduced_potentials = tyff.utils.tensor_like(reduced_potentials, theta[0])
...
class _ReweightAverageOp(torch.autograd.Function):
    ...
    def forward(ctx, kwargs: _ReweightAverageKwargs, *theta: torch.Tensor):
        ...
        with kwargs["frames_path"].open("rb") as file:
            reduced_pot_0 = tyff.utils.tensor_like(
                [v for _, _, v, _ in tyff.mm._reporters.unpack_frames(file)],
                reduced_pot,
            )
    ...
    def backward(ctx, *grad_outputs):
        ...
        for i in range(len(du_d_theta)):
            ...
            log_exp_sum, log_exp_sign = tyff.utils.logsumexp(
