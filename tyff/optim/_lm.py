...
ReportFn = typing.Callable[[int, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, float, bool], None]
...
class LevenbergMarquardtConfig(pydantic.BaseModel):
    ...
    mode: Mode = pydantic.Field(_ADAPTIVE, description="The mode to run the optimizer in.")

    trust_radius: float = pydantic.Field(0.2, description="Target trust radius.", gt=0.0)
    ...
    min_damping_factor: float = pydantic.Field(1.0, description="Minimum damping factor.", gt=0.0)

    adaptive_factor: float = pydantic.Field(
        0.25,
        description="Adaptive trust radius adjustment factor to use when running in adaptive mode.",
        gt=0.0,
    )
    adaptive_damping: float = pydantic.Field(
        1.0,
        description="Adaptive trust radius adjustment damping to use when running in adaptive mode.",
        gt=0.0,
    )
    ...
    search_trust_radius_factor: float = pydantic.Field(
        0.1,
        description="The factor to scale the trust radius by when falling back to a second line search.",
        gt=0.0,
    )
    ...
    convergence_gradient: float = pydantic.Field(
        0.1,
        description="The gradient will be considered converged if its norm is less thanthis value.",
        gt=0.0,
    )
    convergence_step: float = pydantic.Field(
        0.01,
        description="The step size will be considered converged if its norm is less than this value.",
        gt=0.0,
    )
    n_convergence_steps: int = pydantic.Field(
        2,
        description="The number of steps to consider when checking for convergence in the loss.",
    )
    ...
    max_steps: int = pydantic.Field(..., description="The maximum number of full steps to perform.", gt=0)
...
def _damping_factor_loss_fn(
    ...
    _LOGGER.debug(f"finding trust radius: length {dx_norm:.4e} (target {trust_radius:.4e})")
...
def _step(
    ...
    if eigenvalue_smallest < config.min_eigenvalue:
        ...
        adjacency = max(config.min_eigenvalue, 0.01 * abs(eigenvalue_smallest)) - eigenvalue_smallest
        ...
        hessian += adjacency * torch.eye(hessian.shape[0], device=hessian.device, dtype=hessian.dtype)
...
def _reduce_trust_radius(dx_norm: torch.Tensor, config: LevenbergMarquardtConfig) -> torch.Tensor:
    ...
    """
    trust_radius = max(dx_norm * (1.0 / (1.0 + config.adaptive_factor)), config.trust_radius_min)
    _LOGGER.info(f"reducing trust radius to {trust_radius:.4e}")

    return tyff.utils.tensor_like(trust_radius, dx_norm)
...
def _update_trust_radius(
    ...
    if step_quality <= config.quality_threshold_low:
        trust_radius = max(
            ...
            tyff.utils.tensor_like(config.trust_radius_min, dx_norm),
        )
        _LOGGER.info(f"low quality step detected - reducing trust radius to {trust_radius:.4e}")

    elif step_quality >= config.quality_threshold_high and damping_adjusted:
        trust_radius += (
            config.adaptive_factor
            * trust_radius
            * math.exp(-config.adaptive_damping * (trust_radius / config.trust_radius - 1.0))
        )
...
def _has_converged(
    ...
    loss_std = (
        torch.inf if len(loss_history) == 0 else torch.std(torch.stack(loss_history[-config.n_convergence_steps :]))
    )
    loss_converged = len(loss_history) >= config.n_convergence_steps and loss_std < config.convergence_loss
    ...
    return bool(sum((grad_converged, step_converged, loss_converged)) >= config.n_convergence_criteria)
...
def levenberg_marquardt(
    ...
    for step in range(config.max_steps):
        ...
        if loss > (loss_prev + config.error_tolerance):
            ...
            trust_radius = _update_trust_radius(dx_norm, step_quality, trust_radius, damping_adjusted, config)
