...
) -> tuple[tyff.TensorForceField, list[Dimer]]:
...
def compute_dimer_vdw_energy(
    dimer: Dimer, potential: tyff.TensorPotential, ff: tyff.TensorForceField
) -> torch.Tensor:
    ...
    if dimer.dimer_topology.v_sites is not None:
        if dimer.dimer_topology.v_sites.keys:
            dimer_coords = tyff.add_v_site_coords(v_sites=dimer.dimer_topology.v_sites, conformer=dimer.coords, force_field=ff)
        if dimer.mol_a_topology.v_sites.keys:
            mol_a_coords = tyff.add_v_site_coords(v_sites=dimer.mol_a_topology.v_sites, conformer=mol_a_coords, force_field=ff)
        if dimer.mol_b_topology.v_sites.keys:
            mol_b_coords = tyff.add_v_site_coords(v_sites=dimer.mol_b_topology.v_sites, conformer=mol_b_coords, force_field=ff)
